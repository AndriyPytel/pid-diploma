...
class MyGenesicPlotter(AbstractGenesicPlotter):
    ...
    def _mutate(self, process, mutatuion_prop, sigma=1):
        if random.random < mutatuion_prop:
            pid_params = self._get_pid_params
            pid_params = dict((key, random.choise(val, random.gauss(val, sigma))) for (key, val) in pid_params.items())
            

if __name__ == '__main__':

    plotter = MyGenesicPlotter(dt=0.02, population=6, preview=6, batch_size=200)
    plotter.start()
